...
# Loading input data
D <- readFiles("0_data/CC01_business_models.bib", "0_data/CC02_business_models.bib",
...
D <- gsub("\\@", "Manuscript =", D)
    ...
    for (j in 1:length(Tag)) {
      ...
      if (length(POS) == 1) {
        ...
        if (uniqueTag[j] != "C1") {
          ...
          DATA[[uniqueTag[j]]][i] <- paste0(gsub(";", ",", D[POSEND]), collapse = ";")
        }
        if (uniqueTag[j] == "DI") {
          DOI <- gsub("DOI = ", "", D[POS])
  ...
  DATA <- DATA[order(DATA$PY, DATA$AU), ]
  DATA <- DATA[!duplicated(DATA$UT), ]
